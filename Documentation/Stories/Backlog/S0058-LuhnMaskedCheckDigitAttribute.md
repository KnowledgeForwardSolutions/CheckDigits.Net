# S0058: Masked Luhn Check Digit Data Annotation

## Story
**As a** .NET developer  
**I want** a reusable C# data annotation that validates formatted values using the masked Luhn check digit algorithm via **CheckDigits.Net**  
**So that** I can declaratively enforce check digit validation on my model properties without duplicating validation logic.

## Background
Many identifiers (such as credit card numbers and account references) rely on the Luhn algorithm for integrity.  These identifiers are often formatted with additional characters that make them easier for a person to read, but these format characters cause problems for check digit algorithms that expect non-formatted values. **CheckDigits.Net** provides the capability to validate formatted values using the Luhn algorithm and a custom mask. Exposing this capability in data annotation backed by **CheckDigits.Net** enables consistent, maintainable validation using standard .NET validation mechanisms.
...
- The annotation uses **CheckDigits.Net** to validate formatted values according to the Luhn algorithm with a custom mask.
...
- New annotation named MaskedLuhnCheckDigit
- MaskedLuhnCheckDigit will use a generic mask type, i.e. MaskedLuhnCheckDigit<Mask>.
- The generic mask type must have a parameterless constructor to support new Mask().
...
public class Foo
{
    [MaskedLuhnCheckDigit<CustomMask>]
