# S0045: Damm Check Digit Data Annotation

## Story
**As a** .NET developer  
**I want** a reusable C# data annotation that validates values using the Damm check digit algorithm via **CheckDigits.Net**  
...
- The annotation uses **CheckDigits.Net** to validate values according to the Damm algorithm.
- The annotation can be applied to `string` model properties.
- Validation succeeds when the value is valid per the Damm algorithm.
- Validation fails when the value does not pass the Damm check.
- Validation succeeds when the value is `null` or empty and the field is not marked as required.
- A default error message is provided (e.g., *“The field {0} has an invalid Damm check digit*).
...
- New annotation named DammCheckDigit
...
- Generating or calculating Damm check digits (validation only).
- Support for non-Damm check digit algorithms.
