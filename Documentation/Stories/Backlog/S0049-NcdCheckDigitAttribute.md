...
## Story
**As a** .NET developer  
**I want** a reusable C# data annotation that validates values using the NOID check digit algorithm via **CheckDigits.Net**  
...
- Validation succeeds when the value is valid per the Ncd algorithm.
- Validation fails when the value does not pass the Ncd check.
- Validation succeeds when the value is `null` or empty and the field is not marked as required.
- A default error message is provided (e.g., *“The field {0} has invalid Ncd check digits*).
...
- New annotation named NcdCheckDigit
...
- Generating or calculating Ncd check digits (validation only).
- Support for non-Ncd check digit algorithms.
