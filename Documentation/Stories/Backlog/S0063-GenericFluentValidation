...
**So that** I can declaratively enforce check digit validation on my model validators without duplicating validation logic.
...
- New CheckDigitValidator<TAlgorithm> class that derives from PropertyValidator
- New extension CheckDigit<TAlgorithm> method that adds the check digit validator to the current rule builder
...
## Definition of Done
- New CheckDigit<TAlgorithm> method demonstrated to work as expected
