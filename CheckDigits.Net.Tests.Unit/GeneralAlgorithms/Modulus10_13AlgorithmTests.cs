...
public class Modulus10_13AlgorithmTests
{
   ...
   [Theory]
   [InlineData("000 000 000 017")]
   [InlineData("000 000 001 007")]
   [InlineData("000 000 100 007")]
   public void Modulus10_13Algorithm_ValidateMasked_ShouldCorrectlyWeightOddPositionCharacters(String value)
      => _sut.Validate(value, _groupsOfThreeMask).Should().BeTrue();

   [Theory]
   [InlineData("000 000 000 109")]
   [InlineData("000 000 010 009")]
   [InlineData("000 001 000 009")]
   public void Modulus10_13Algorithm_ValidateMasked_ShouldCorrectlyWeightEvenPositionCharacters(String value)
      => _sut.Validate(value, _groupsOfThreeMask).Should().BeTrue();

   [Theory]
   [InlineData("036 000 291 452")]           // Worked UPC-A example from Wikipedia (https://en.wikipedia.org/wiki/Universal_Product_Code#Check_digit_calculation)
   [InlineData("425 261")]                   // UPC-E example
   [InlineData("400 638 133 393 1")]         // Worked EAN-13 example from Wikipedia (https://en.wikipedia.org/wiki/International_Article_Number)
   [InlineData("735 135 37")]                // Worked EAN-8 example from Wikipedia
   [InlineData("978 050 051 695 9")]         // ISBN-13, Islamic Geometric Design, Eric Broug
   [InlineData("012 345 678 000 045 678")]   // Example SSCC number
   public void Modulus10_13Algorithm_ValidateMasked_ShouldReturnTrue_WhenValueContainsValidCheckDigit(String value)
      => _sut.Validate(value, _groupsOfThreeMask).Should().BeTrue();
   ...
   [InlineData("012 345 876 000 045 678")]   // SSCC number with jump transposition error (678 -> 876)
   public void Modulus10_13Algorithm_ValidateMasked_ShouldReturnTrue_WhenValueContainsUndetectableError(String value)
      => _sut.Validate(value, _groupsOfThreeMask).Should().BeTrue();

   [Theory]
   [InlineData("036 000 391 452")]           // UPC-A with single digit transcription error (2 -> 3)
   [InlineData("427 261")]                   // UPC-E with single digit transcription error (5 -> 7)
   [InlineData("400 638 313 393 1")]         // EAN-13 with two digit transposition error (13 -> 31)
   [InlineData("987 050 051 695 9")]         // ISBN-13 with two digit transposition error (78 -> 87)
   public void Modulus10_13Algorithm_ValidateMasked_ShouldReturnFalse_WhenInputContainsDetectableError(String value)
      => _sut.Validate(value, _groupsOfThreeMask).Should().BeFalse();

   [Fact]
   public void Modulus10_13Algorithm_ValidateMasked_ShouldReturnTrue_WhenInputIsAllZeros()
      => _sut.Validate("000 000 000 000", _groupsOfThreeMask).Should().BeTrue();

   [Fact]
   public void Modulus10_13Algorithm_ValidateMasked_ShouldReturnTrue_WhenCheckDigitIsCalculatesAsZero()
      => _sut.Validate("000 000 090 100", _groupsOfThreeMask).Should().BeTrue();
